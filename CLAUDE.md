- Add robust Markdown documentation for every feature as its built to the /docs folder - anything admins needs to do in infrastructure via artisan commands, any set up required, any user notes, agent documentation, etc.
- Always use the latest version of Pest for unit tests, following their documentation for best practices
- Always use the custom FormSelect or InlineSelect components for dropdowns - never use native HTML select elements

## Deployment
- This project uses **Laravel Cloud** for hosting and deployment
- Pushing to `main` branch triggers automatic deployment
- No manual deployment steps required - Laravel Cloud handles build, migrations, and queue restart
